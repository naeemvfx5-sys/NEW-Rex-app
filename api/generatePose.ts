...
const handler = async (req: VercelRequest, res: VercelResponse) => {
    ...
    try {
        const { baseImage, options } = req.body;
        ...
        if (mode === 'image') {
            if (!poseImage) {
                return res.status(400).json({ error: "A pose image is required when using image mode." });
            }
            instructionPrompt = `
        ...
        if (firstPart?.text?.includes("Pose detection failed")) {
            return res.status(400).json({ error: "Pose detection failed — please upload a clearer pose reference image." });
        }
        ...
        return res.status(500).json({ error: "No image was generated by the API. The model may have refused the request." });
